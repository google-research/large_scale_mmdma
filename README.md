...
- `--data`: str, it can be either 'branch'(default) or 'triangle'. The simulated
data is described in the pydoc of [data_pipeline.generate_data](https://github.com/google-research/large_scale_mmdma/blob/master/lsmmdma/data/data_pipeline.py).
- `--n`: int (default 3000), number of samples for both views.
- `--p`: int (default 1000), number of features for both views.

For data given by the user, the input flags are:
- `--input_dir`: str (default ''), input directory.
- `--input_fv`: str (default ''), filename of the array (n1 x p1) that serves as
first set of points.
- `--input_sv`: str (default ''), filename of the array (n2 x p2) that serves as
second set of points.
- `--rd_vec`: str (default ''), filename of the vector that contains the indices
...
*Seeds* The seed is fixed with the flag `--seed` (int, default value is 0). In the
case one wishes to use multipe starts (X seeds and selection of the best one
based on the value of the loss), it is possible to also define the number of
seeds to try with: `--ns` (int, default value is 1).
...
- `--e`: int (default 10001), number of epochs for the training process.
